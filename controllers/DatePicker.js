...
class DatePicker extends TelegramBaseController {
  ...
  generateMenu($, monthnum = "") {
    ...
    const genMenus = [
      {
        text: monthText + ` ${year}`,
        callback: callbackQuery => this.callbackRes(callbackQuery)
      },
      {
        text: "Su",
        callback: callbackQuery => this.callbackRes(callbackQuery)
        ...
        if (monthCalendar[i][j] === 0) {
          menu = {
            ...
            callback: callbackQuery => {
              this.callbackRes(callbackQuery);
            }
          };
        } else {
          menu = {
  ...
  callbackRes(callbackQuery, res) {
    if (!res)
      return bot.api.answerCallbackQuery(callbackQuery.id, {
        text: "Not a Valid option, Please pick a day",
        show_alert: true
      });
