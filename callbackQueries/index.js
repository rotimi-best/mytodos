...
class CallbackQuery extends TelegramBaseCallbackQueryController {
  ...
  saveTodoFromInlineQuery(inlineMsgId, { id, firstName }) {
    ...
    fs.readFile(
      `${process.cwd()}/.data/inlinemode.txt`,
      "utf8",
      async (err, data) => {
        if (err) {
          ...
          if (len(todoArray)) {
            ...
            if (len(todoToAdd)) {
              ...
              bot.api.editMessageText(
                `Great ${firstName} ${thumbsUp}, I have added _${todoToAdd}_ to your todo list.\n[View in bot](${BOT_LINK})`,
                {
  ...
  dontSaveTodoFromInlineQuery(inlineMsgId, { firstName }) {
    bot.api.editMessageText(
      `Okay ${firstName} ${thumbsUp}, I didn't add it to your todo list`,
      {
        inline_message_id: inlineMsgId
      }
    );
